...
function rest_get_tax_terms($post)
{
	...
	$selected = isset($post['selected']) ? wc_clean($post['selected']) : false;
...
function rest_update_term_swatch($post)
{
	...
	$type = isset($post['type']) ? sanitize_text_field($post['type']) : '';
	$more = isset($post['more']) ? wc_clean($post['more']) : '';
...
function get_product_attributes($product)
{
	...
	foreach ($attributes as $attribute_name => $attr_options) {
		...
		$attr_data[$key]['selected'] = isset($_REQUEST[$attr_data[$key]['name']]) ? wc_clean($_REQUEST[$attr_data[$key]['name']]) : $attr_data[$key]['default']; // phpcs:ignore WordPress.Security.NonceVerification.Recommended
